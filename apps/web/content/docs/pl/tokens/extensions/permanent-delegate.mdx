...
przypisuje delegata z uprawnieniami do wszystkich token accountów dla danego
mintu. W przeciwieństwie do zwykłych delegatów, których właściciele token
accountów mogą odwołać, stałego delegata nie można zmienić ani usunąć przez
właścicieli token accountów. Stały delegat może transferować lub spalać tokeny
na dowolnym token accountcie tego mintu. Rola stałego delegata może zostać
przekazana nowemu kontu przez obecnego stałego delegata.
